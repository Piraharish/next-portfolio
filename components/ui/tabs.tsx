...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.List
    ref={ref}
    className={cn(
      "flex md:flex-col max-md:w-full h-max items-start justify-safe border-2 border-transparent max-md:border-b-muted md:border-l-muted pr-4 text-muted-foreground max-md:overflow-x-scroll no-scrollbar",
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.Content
    ref={ref}
    className={cn("mt-2 custom-focus", className)}
    {...props}
  />
));
